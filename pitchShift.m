...
% DESCRIPTION
%
% The following code is taken from guitarpitchshifter.com and can be 
% understood by the 3 main sections..
%
% Analysis, Processing and Synthesis.
%
% ANALYSIS
% In order to acheive the pitch synchronous overlap add algorithm the input
% signal 'x' is first split into frames using the function 'createFrames()',
% this returns a matrix ('y') with a row for each frame and the number of
% frames, 'numberFramesInput'. The frames overlap by 75% giving them 
% 1/4 hop size, 'hop'. The signal is expanded or compressed by the value defined 
% in the variable 'alpha' and then using interpolation, played back by the
% same degree.
%
% A for loop is used to apply a hann window to each frame this reduces
% the windowing precedures affect on the frequency domain and centers most 
% energy at dc. An FFT is taken and the magnitude and phase is stored in the 
% variables 'magsFrame' and  'phaseFrame'. 
%
% PROCESSING
% A phase vocoder is used to remove phase discontinuity between frames improving the
% the acuraccy of fft by reducing smearing between bins. 
% The phase difference between successive frames is calculated and stored
% in the variable 'deltaPhi' this is removed from the expected phase in 
% 'deltaPhiPrime' and mapped between -pi and pi, 'deltaPhiPrimeMod'. Given
% this variable and the bin frequencies the true frequency is stored in 
% 'truefreq'. The total phase offset of all frames is stored in 'phaseCumulative'.
% 
% SYNTHESIS
% An IFFT is performed to bring the frame back to the time domain and a
% hann window is applied to crossfade the signal between successive frames.
% 
% After the loop the fusionFrames() function is used to create a single vector
% for output, 'outputTimeStretched'. linear interpolation is then used to
% resample the vector adjusting the playback speed appropriately, 'outputVector'.
%
% EXAMPLE
%
% Apply a pitch shifter effect to an audio sample test.wav.
%
% [x, fs] = audioread('test.wav');   read audio file
% windowSize  = 1024;                assign window size
% hopSize = 256;                     assign hop size 1/4
% step = 2;                         assign how many semitones to shift by 
%  
% y = pitchshifter(x, windowSize, hopSize, step);    run function    
% 
